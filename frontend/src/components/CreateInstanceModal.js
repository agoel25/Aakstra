...
const instanceTypes = [
  { type: 'small', cost: '$0.01/hr' },
  { type: 'medium', cost: '$0.05/hr' },
  { type: 'large', cost: '$0.10/hr' },
];
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      <select
        ...
        onChange={(e) => setSelectedInstanceType(instanceTypes.find(instance => instance.type === e.target.value))}
        className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline mb-4"
      >
        {instanceTypes.map(instance => (
          <option key={instance.type} value={instance.type}>
            {`${instance.type.charAt(0).toUpperCase() + instance.type.slice(1)} - ${instance.cost}`}
          </option>
        ))}
      </select>
      <button
